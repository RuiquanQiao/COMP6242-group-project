...
The data-size experiment adds a sample-efficiency perspective. In Section 4.3, `full_ft` gives its largest transfer gain at the 10% EuroSAT training fraction, improving top-1 accuracy by 25.56 percentage points over scratch. As more labelled EuroSAT data become available, the scratch baseline improves and the relative gain from full fine-tuning decreases to about 7-8 points. This suggests that ImageNet pretraining is most valuable when target labels are limited, because the pretrained representation reduces the amount of task-specific data needed to reach high accuracy.

However, Section 5.3 shows that this sample-efficiency benefit does not remove the forgetting cost. `full_ft` causes severe ImageNet forgetting at every training fraction, with ImageNet top-1 accuracy dropping to roughly 0.20%-1.24% after EuroSAT adaptation. By contrast, `linear_probe` preserves more ImageNet performance but gives lower EuroSAT accuracy. The data-size results therefore reinforce the same trade-off observed in the strategy and domain-gap experiments: stronger downstream adaptation improves the new task, especially in low-data settings, but it does not preserve the original ImageNet task.
