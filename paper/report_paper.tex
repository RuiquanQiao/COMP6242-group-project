...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.86\columnwidth]{../outputs/eurosat_ablation/test_top1_acc.png}
\vspace{0.35em}
\includegraphics[width=0.86\columnwidth]{../outputs/eurosat_ablation/val_top1_acc_curve.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\columnwidth]{../outputs/domain_gap/test_top1_acc.png}
\vspace{0.1em}
\includegraphics[width=0.78\columnwidth]{../outputs/domain_gap/transfer_gain_top1.png}
\caption{Same-size cross-domain comparison. Top: downstream test top-1 accuracy. Bottom: top-1 transfer gain over training from scratch.}
\label{fig:domain}
\end{figure}
...
This comparison separates absolute accuracy from transfer benefit. EuroSAT reaches higher final accuracy, but CIFAR-10 receives the larger gain over training from scratch. This observation highlights that absolute task performance and transferability are distinct concepts. A task may achieve higher final accuracy while benefiting less from pretraining if it can already be learned effectively from scratch.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\columnwidth]{../outputs/data_fraction/test_top1_acc.png}
\vspace{0.1em}
\includegraphics[width=0.78\columnwidth]{../outputs/data_fraction/transfer_gain_top1.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_main/forgetting_top1.png}
\vspace{0.35em}
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_main/transfer_forgetting_tradeoff.png}
\caption{Forgetting after the main EuroSAT experiment. Top: ImageNet top-1 forgetting. Bottom: downstream accuracy and forgetting trade-off.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_domain_gap/forgetting_top1.png}
\vspace{0.35em}
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_domain_gap/transfer_forgetting_tradeoff.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_data_fraction/forgetting_top1_transfer_methods.png}
\vspace{0.35em}
\includegraphics[width=0.90\columnwidth]{../outputs/forgetting_data_fraction/transfer_forgetting_tradeoff.png}
...
This explains the trade-off plots in Sections~5.1 and~5.2. Linear probing appears on the lower-forgetting side because the pretrained backbone is preserved, but it does not reach the highest downstream accuracy because only the classifier head can adapt. Partial unfreezing and full
fine-tuning move toward higher downstream accuracy, but they also move to the higher-forgetting side because many more backbone parameters are updated. The experiments therefore do not find a strategy in the ideal upper-left region of the trade-off plots, where downstream accuracy would be high and ImageNet forgetting would be low.

The study has several limitations. First, all experiments use a single architecture (ResNet-18), so the conclusions may not generalize to larger CNNs or transformer-based models. Second, all results are reported using a single random seed, making it difficult to assess statistical variability. Third, we intentionally keep the training budget fixed at eight epochs for fairness, but some strategies may benefit from longer optimization. Finally, the forgetting analysis relies on direct ImageNet re-evaluation and does not compare mitigation approaches such as EWC, Learning without Forgetting, rehearsal methods, or progressive freezing \cite{kirkpatrick2017,li2016}.
